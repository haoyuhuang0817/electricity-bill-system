...
def generateBill(userid, logintime):
    ...
    if consumerno == [] or consumerno == None:
        ...
        print('OR')
        print('The data was generated already for this month!')
    ...
    for x, y in consumerno:
        ...
        db.execute(f'UPDATE customer SET amountgen={amountgen}, rebate={rebate}, aduj={aduj} WHERE unit_consumed={x} AND consumerno={y}')
        connection.commit()
...
def Bill_Calc1(unit):
    ...
    if unit >= 1 and unit <= 50:  # between 1 - 50 units
        ...
        amountgen = ceil((50 * 4.89) + (unit - 50) * 5.4) + meter + MVCA + fixedChrge + int(untstr) - 1.4
        rebate = 1.4
        aduj = untstr
